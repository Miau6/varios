...
aborto <- aborto %>% clean_names()
aborto$si_una_instancia_hizo_una_referencia_al_servicio <- str_replace(aborto$si_una_instancia_hizo_una_referencia_al_servicio, 
                       "ESPACIALIZADO", "ESPECIALIZADO")


aborto <- aborto %>% #clean_names() %>% 
  mutate(semanas_de_gestacion=round(semanas_de_gestacion),
         rango_edad=case_when(
           edad<15 ~ "Menor a 16", 
           edad>15 & edad<=17 ~ "16 a 17", 
           edad>17 & edad<=30 ~ "18 a 30", 
           edad>=31 & edad<=39 ~ "31 a 39", 
           edad>=40 ~ "Mayor a 40"
         ), rango_edad=factor(rango_edad, 
                              levels=c("Menor a 16", "16 a 17", 
                                       "18 a 30", "31 a 39", "Mayor a 40")), 
         si_va_sola_o_acompanada=str_to_sentence(si_va_sola_o_acompanada), 
         indigena=ifelse(si_es_hablante_de_alguna_lengua_indigena=="NO", "NO", "SI"), 
         causal=case_when(
           grepl("Riesgo", causal_por_la_que_se_llevo_a_cabo) ~ "Riesgo a la salud", 
           T ~ causal_por_la_que_se_llevo_a_cabo
         ), 
         violencia=ifelse(si_se_le_realizo_una_deteccion_de_violencia=="SI", "SI", "NO"), 
         ocupacion=str_to_sentence(ocupacion), 
         escolaridad=case_when(
           grepl("sica", escolaridad) ~ "Básica", 
           T ~ str_to_sentence(escolaridad)
         ), 
         referencia=case_when(
           grepl("FEMINIS", si_una_instancia_hizo_una_referencia_al_servicio) ~ "GRUPO FEMINISTA", 
           grepl("PRIVA", si_una_instancia_hizo_una_referencia_al_servicio) ~ "CONSULTA PRIVADA", 
           si_una_instancia_hizo_una_referencia_al_servicio %in% c("SERVCIO ESPECIALIZADO", 
                                                                   "SERVICIO ESPECILIZADO") ~ "SERVICIO ESPECIALIZADO", 
           T ~ si_una_instancia_hizo_una_referencia_al_servicio
         ))
...
ui <- shinyUI(
  ...
  transition-delay: 0;
      ...
      navbarPage(title = "Datos abiertos",
                 ...
                 shiny::navbarMenu(title = "Incidencia delictiva", #icon = icon("dot-circle"),
                                   shiny::tabPanel(title = "Fiscalía Nayarit",
                              #icon = icon("dot-circle"),
                               # tabPanel(title = "Municipal",
                              ...
                              shiny::tabPanel(title = "Secretariado Ejecutivo del Sistema Nacional Seguridad Pública (SESNSP)",
                                     #icon = icon("dot-circle"),
                                     # tabPanel(title = "Municipal",
                                     #tabPanel("Total por municipio",
                                     #box(width=12,
                 ...
                 shiny::navbarMenu(title = "Atención a víctimas", #icon = icon("dot-circle"),
                                   shiny::tabPanel(title = "Comisión Ejecutiva de Atención a Víctimas",
                                     #box(width=12,
                                         div(class="row d-flex", #Replicar
...
# 
  ...
  output$grafico_cebp_3<- renderPlotly({
    ...
    facet_wrap(~`estado civil`, nrow = 3)+
    labs(y="", x="")+
    theme_minimal()+
...
fiscalia_data() %>%
  ...
  data_aborto <- reactive({
    aborto %>% 
      filter(fecha>=min(input$aborto_date), fecha<=max(input$aborto_date), 
             if(is.null(input$aborto_indigena)) indigena!="" else indigena %in% input$aborto_indigena,
             if(is.null(input$aborto_edad)) rango_edad!="" else rango_edad %in% input$aborto_edad,
             if(is.null(input$aborto_causal)) causal!="" else causal %in% input$aborto_causal,
             if(is.null(input$aborto_violencia)) violencia!="" else violencia %in% input$aborto_violencia
  ...
  data_lesiones <- reactive({
    ...
    output$gr_aborto_date <- renderPlotly({
      plot <- data_aborto() %>% 
        ...
        complete(Mes=seq.Date(as_date(min(input$aborto_date)),
                                as_date(max(input$aborto_date)), "month"), 
                 fill=list(Total=0)) %>% 
      ...
      ggplotly(plot#, tooltip="text"
        ...
        layout(legend = list(orientation = "h", x = 0.1, y = -0.8),
               margin = list(b=0,t=30),
               title = paste0("Total interrupciones legales de embarazo")
        )
    })
    ...
    output$gr_aborto_causal<- renderPlotly({
      plot <- data_aborto() %>% 
        group_by(causal=str_wrap(causal, 12)) %>% 
        summarise(Total=n()) %>% 
        mutate(text=paste0("Causa: ", causal, "\n", 
                           "Total: ", comma(Total))) %>% 
        ggplot(aes(reorder(causal, -Total), Total, fill=causal, text=text)) +
        geom_col() + theme_light() +
    ...
    # [5] "#B16548" "#8D6B86" "#8068AE" "#A850A0"
    # [9] "#D03792" "#D33B79" "#A66753" "#79932E"
    # [13] "#7FA718" "#ACA80E" "#D9AA04" "#D59D08"
    # [17] "#BF8B12" "#A9781B" "#927132" "#7C6B4C"
    # [21] "#666666"
